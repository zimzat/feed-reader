...
!function(angular) {
    ...
    angular.module("Reader.CategoryList").controller("Reader.CategoryList.Controller", function($scope, $resource, $location, hotkeys, config) {
        ...
        $scope.summary = Category.get(), hotkeys.bindTo($scope).add({
            combo: "p",
    ...
    angular.module("Reader.EntryList").controller("Reader.EntryList.Controller", function($scope, $resource, $routeParams, $location, hotkeys, config, ListResult) {
        ...
        } else $routeParams.feedId && (Category = $resource(config.apiUrl + "/feed/:feedId/entry", {
            feedId: $routeParams.feedId
        }));
        $scope.entries = Category.query(), $scope.action = {
            seek: function(index) {
    ...
    angular.module("Reader.EntryView").controller("Reader.EntryView.Controller", function($scope, $resource, $routeParams, $location, hotkeys, config, ListResult) {
        ...
        $scope.entry = Entry.get({}, function() {
            $scope.entry.isRead || $scope.action.toggleRead();
        }), $scope.action = {
            goPrevious: function() {
                ListResult.isBeginning() ? $location.url("/") : $location.url("/entry/" + ListResult.previous().entryId);
            },
            goNext: function() {
                ListResult.isEnd() ? $location.url("/") : $location.url("/entry/" + ListResult.next().entryId);
            },
    ...
    angular.module("Reader.FeedList").controller("Reader.FeedList.Controller", function($scope, $resource, $routeParams, $location, hotkeys, config) {
        ...
        $scope.feeds = Category.query(), hotkeys.bindTo($scope).add({
            combo: "p",
            description: "Previous: Go back to main category listing",
            callback: function() {
                $location.url("/");
    ...
    angular.module("Reader.Login").controller("Reader.Login.Controller", function($scope, $location, hotkeys) {
        $location.url("/category"), hotkeys.bindTo($scope).add({
            combo: "enter",
            callback: function() {
