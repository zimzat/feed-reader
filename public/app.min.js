/*! [prod] reader 2015-07-03 03:34:50 */
!function(angular) {
    "use strict";
    ...
    angular.module("Reader").run(function($location, $resource, hotkeys, config) {
        hotkeys.add({
            combo: "l",
            description: "Logout",
            callback: function() {
                $resource(config.apiUrl + "/authentication/logout").save(), delete sessionStorage.authToken, 
                $location.url("/login");
            }
        });
    }).constant("env", "prod").constant("config", function() {
        return {
    ...
    angular.module("Reader.CategoryList").controller("Reader.CategoryList.Controller", function($scope, $resource, $location, $timeout, hotkeys, config) {
        var updatePromise, Category = $resource(config.apiUrl + "/category"), updateFunction = function() {
            $scope.summary = Category.get({}, function() {
                updatePromise = $timeout(updateFunction, 3e5, !1);
            });
        };
    ...
    var fullScreenSlideshow = function($, $rootScope, ListResult) {
        ...
        function nextImg() {
            hasNext() ? resizeAndRotateImages() : operations.goNextEntry();
        }
        function enable() {
    ...
    angular.module("Reader").run(function($rootScope, $location) {
        $rootScope.$on("$locationChangeStart", function(event, newLocation) {
            -1 !== newLocation.indexOf("login") || sessionStorage.authToken || (event.preventDefault(), 
            $location.url("/login"));
        });
    }).service("AuthInterceptor", function($q, $location) {
        var service = this;
        service.request = function(config) {
            return sessionStorage.authToken && (config.headers.Authorization = "Token " + sessionStorage.authToken), 
            config;
        }, service.response = function(config) {
            return config.headers("Authorization") && (sessionStorage.authToken = config.headers("Authorization")), 
            config;
        }, service.responseError = function(response) {
            return 401 === response.status && -1 === response.config.url.indexOf("login") && (delete sessionStorage.authToken, 
